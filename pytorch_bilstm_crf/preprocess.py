...
def read_corpus(corpus_path):
    ...
    # 逐行读取语料文件
    # for line in tqdm.tqdm(open(corpus_path, encoding='utf-8')):
...
def build_data_loader(dataset, batch_size = None, shuffle = False):
    ...
    def collate_batch(batch):
        ...
        for (_text, _label) in batch:
            ...
            mask = torch.ones_like(processed_label).bool()
        ...
        return text_list.to(device), label_list.to(device), mask_list.to(device)

    # 构建数据加载器
    return DataLoader(dataset, batch_size=batch_size, shuffle=shuffle, collate_fn=collate_batch)

if __name__ == '__main__':
    train_file = os.path.abspath('BIO/trainBIO/modelA.txt.txt')
    tag_file = os.path.abspath('BIO/tags_A.txt')
    tokens,tags = read_corpus(train_file)
    print('训练语料大小:',len(tokens))
    vocab = build_vocab(tokens)
    print('token字典大小:',len(vocab))
    tag_to_idx = build_tag_dict(tag_file)
    print(tag_to_idx)    
    train_ds = NerDataSet(tokens, tags, vocab, tag_to_idx)
    train_dl = build_data_loader(train_ds, batch_size=4, shuffle=True)
    for token_idx, label_idx, mask in train_dl:
        print(token_idx.shape, label_idx.shape, mask.shape)
        print(token_idx[0])
        print(label_idx[0])
        print(mask[0])
        break
